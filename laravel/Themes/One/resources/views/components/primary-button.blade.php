<<<<<<< HEAD
 {{--
    ATTENZIONE: Questo componente non deve mai essere vuoto!
    Ogni button UI deve avere uno stile di base robusto per evitare problemi di visibilità.
    Se vuoi personalizzare, estendi o sovrascrivi, ma non lasciare mai vuoto.
--}}
<button {{ $attributes->merge(['type' => 'submit', 'class' => 'inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150']) }}>
    {{ $slot }}
</button>
=======
 
>>>>>>> 1b374b6 (.)
