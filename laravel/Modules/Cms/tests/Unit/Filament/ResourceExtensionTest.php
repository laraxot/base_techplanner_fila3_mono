...
test('page resource form schema contains expected fields', function () {
    ...
    $hasTitleField = collect($schema)->contains(fn ($field) => is_object($field) && method_exists($field, 'getName') && $field->getName() === 'title'
    );

    $hasSlugField = collect($schema)->contains(fn ($field) => is_object($field) && method_exists($field, 'getName') && $field->getName() === 'slug'
    );
...
test('menu resource form schema contains expected fields', function () {
    ...
    $hasItemsField = collect($schema)->contains(fn ($field) => is_object($field) && method_exists($field, 'getName') && $field->getName() === 'items'
    );
