...
trait HasComments
{
    ...
    {
        return $this->morphMany(Config::commentModelClass(), 'commentable');
    }

    public function notificationSubscriptions(): MorphMany
    {
        return $this->morphMany(Config::commentNotificationSubscriptionModelClass(), 'commentable');
    }
    ...
    public function comment(string $text, ?CanComment $commentator = null): Comment
    {
        ...
        if (! Config::allowAnonymousComments()) {
            if (! $commentator) {
                throw CannotCreateComment::userIsRequired();
            }
        }

        $parentId = ($this::class === Config::commentModelClass())
            ? $this->getKey()
            : null;
