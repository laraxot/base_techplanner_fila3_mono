...
class Comment extends Model
{
    ...
    {
        return $this->belongsTo(Config::commentModelClass(), 'parent_id');
    }

    public function nestedComments(): HasMany
    {
        return $this->hasMany(Config::commentModelClass(), 'parent_id');
    }
    ...
    {
        return $this->hasMany(Config::reactionModelClass());
    }
    ...
    public function participatingCommentators(): Collection
    {
        return Comment::query()
            ...
            ->flatMap(
                fn (array $ids, string $class) =>
                    (str_contains($class, '\\') ? $class : Relation::morphMap()[$class])::query()
    ...
    {
        if (Config::automaticallyApproveAllComments()) {
            return true;
        }
    ...
    public function getMentionees(): Collection
    {
        ...
        preg_match_all('/data-mention="([^"]+)"/', $text, $matches);
        $mentioneeIds = $matches[1];

        $modelClass = Config::commentatorModelClass();

        if (! $modelClass) {
            return collect();
        }

        return $modelClass::find($mentioneeIds);
    }
}
