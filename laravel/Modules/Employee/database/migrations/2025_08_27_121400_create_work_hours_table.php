...
return new class extends XotBaseMigration {
    ...
    public function up(): void
    {
        ...
        $this->tableCreate(static function (Blueprint $table): void {
            ...
            $table->dateTime('timestamp')->comment('Exact time of entry');

            $table->decimal('location_lat', 10, 8)->nullable()->comment('GPS latitude coordinate');

            $table->decimal('location_lng', 11, 8)->nullable()->comment('GPS longitude coordinate');

            $table->string('location_name')->nullable()->comment('Human readable location name');

            $table->json('device_info')->nullable()->comment('Device information for tracking');

            $table->string('photo_path')->nullable()->comment('Path to verification photo');

            $table->text('notes')->nullable()->comment('Optional notes for entry');

            $table->enum('status', ['pending', 'approved', 'rejected'])->default('pending');

            $table->foreignId('approved_by')->nullable()->constrained('users')->onDelete('set null');

            $table->dateTime('approved_at')->nullable()->comment('When entry was approved');
