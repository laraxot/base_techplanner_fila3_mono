...
class ListClients extends XotBaseListRecords
{
    ...
    public null|int $selectedClientId = null;
    protected null|\Illuminate\Database\Eloquent\Builder $tableQuery = null;
    ...
    {
        $columns = [
            'distance' => TextColumn::make('distance')->formatStateUsing(fn($state) => number_format($state, 2) .
            ' km'),
            ...
            ->formatStateUsing(function (Client $record): string {
                ...
                if ($record->phone) {
                    $contacts[] =
                        '<a href="tel:' .
                        $record->phone .
                        '" class="inline-flex items-center text-blue-600 hover:text-blue-800 mr-2 mb-1" title="Telefono: ' .
                        $record->phone .
                        '">
                ...
                if ($record->email) {
                    $contacts[] =
                        '<a href="mailto:' .
                        $record->email .
                        '" class="inline-flex items-center text-green-600 hover:text-green-800 mr-2 mb-1" title="Email: ' .
                        $record->email .
                        '">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                if ($record->pec) {
                    $contacts[] =
                        '<a href="mailto:' .
                        $record->pec .
                        '" class="inline-flex items-center text-purple-600 hover:text-purple-800 mr-2 mb-1" title="PEC: ' .
                        $record->pec .
                        '">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                if ($record->whatsapp) {
                    ...
                    $contacts[] =
                        '<a href="https://wa.me/' .
                        $cleanWhatsapp .
                        '" target="_blank" class="inline-flex items-center text-green-500 hover:text-green-700 mr-2 mb-1" title="WhatsApp: ' .
                        $record->whatsapp .
                        '">
    ...
    public function getTableFilters(): array
    {
        $activities = static::getModel()::query()
            ...
            ->map(app(\Modules\Xot\Actions\Cast\SafeStringCastAction::class)->execute(...))
            ->toArray();
        ...
        return [
            ...
            TernaryFilter::make('business_closed')->default(false)->label('Attività chiusa'),
    ...
    public function getHeaderActions(): array
    {
        ...
        $actions = [
            ...
            Actions\ImportAction::make('importClient')->importer(ClientImporter::class),
    ...
    private function formatContacts(Client $record): string
    {
        ...
        if ($record->mobile) {
            $contacts[] =
                '<i class="heroicon-o-device-phone-mobile text-purple-500 w-4 h-4 inline mr-1" title="Cellulare"></i> ' .
                $record->mobile;
        }
        ...
        if ($record->email) {
            $contacts[] =
                '<i class="heroicon-o-envelope text-green-500 w-4 h-4 inline mr-1" title="Email"></i> ' .
                $record->email;
        }
        ...
        if ($record->pec) {
            $contacts[] =
                '<i class="heroicon-o-shield-check text-orange-500 w-4 h-4 inline mr-1" title="PEC"></i> ' .
                $record->pec;
        }
        ...
        if ($record->whatsapp) {
            $contacts[] =
                '<i class="fab fa-whatsapp text-green-600 w-4 h-4 inline mr-1" title="WhatsApp"></i> ' .
                $record->whatsapp;
        }
        ...
        if ($record->fax) {
            $contacts[] =
                '<i class="heroicon-o-printer text-gray-500 w-4 h-4 inline mr-1" title="Fax"></i> ' . $record->fax;
        }
    ...
    protected function getTableQuery(): Builder
    {
        ...
        return $query->when($latitude && $longitude, function (Builder $query) use ($latitude, $longitude) {
            /** @phpstan-ignore-next-line */
            $query->withDistance($latitude, $longitude)->orderByDistance($latitude, $longitude);
        });
    }
}
