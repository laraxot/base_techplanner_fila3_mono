    ...
    function dddx(mixed $params): string
    {
        ...
        if (
            File::exists($data['file']) &&
                Str::startsWith(
                    $data['file'],
                    app(Modules\Xot\Actions\File\FixPathAction::class)->execute(storage_path('framework/views')),
                )
        ) {
            // $data['extra'] = 'preso';
            $content = File::get($data['file']);
            $data['view_file'] = app(Modules\Xot\Actions\File\FixPathAction::class)
    ...
    function getAllModulesModels(): array
    {
        ...
        foreach ($modules as $module) {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::error('[Module:' .
                $moduleName .
